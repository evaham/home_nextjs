    ...
    <main>
      ...
      <div className="w-[30rem] mx-auto flex flex-col py-14 lg:w-[80rem] lg:flex-row">
        <ul className="flex flex-row py-4 lg:flex-col items-start lg:w-2/5 mr-8" >
          {tab.map((tab)=>{
            return (
              <li key={tab.id} onClick={() => handleTabClick(tab.cate)} className={`font-bold rounded-full m-2 px-6 py-2 hover:cursor-pointer ${selectedTab === tab.cate ? 'bg-blue-900 text-white' : 'text-slate-500 bg-slate-200'}`}>{tab.name}</li>
            )
          })}
        </ul>
